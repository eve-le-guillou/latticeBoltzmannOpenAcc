...
#include <errno.h>
#include "CpuFunctions.h"       // CPU kernels
...
int Iterate2D(InputFilenames *inFn, Arguments *args) {
	...
	*bcType, *bcBoundId,*tempi;
	...
	FLOAT_TYPE *p_in = createHostArrayFlt(n*m, ARRAY_ZERO);
    FLOAT_TYPE *p_out = createHostArrayFlt(n*m, ARRAY_ZERO);
    int *num_in = createHostArrayInt(n*m, ARRAY_ZERO);
    int *num_out = createHostArrayInt(n*m, ARRAY_ZERO);
	...
	if (args->inletProfile == NO_INLET) {
        ...
        u0 = createHostArrayFlt(m * n, ARRAY_FILL, args->u);
        v0 = createHostArrayFlt(m * n, ARRAY_FILL, args->v);
	} else {
    ...
    FLOAT_TYPE *drag = createHostArrayFlt(m * n, ARRAY_ZERO);
    FLOAT_TYPE *lift = createHostArrayFlt(m * n, ARRAY_ZERO);

    FLOAT_TYPE *f = createHostArrayFlt(9 * m * n, ARRAY_ZERO);
    FLOAT_TYPE *fColl = createHostArrayFlt(9 * m * n, ARRAY_ZERO);

    FLOAT_TYPE *temp9a = createHostArrayFlt(9 * m * n, ARRAY_ZERO);
    FLOAT_TYPE *temp9b = createHostArrayFlt(9 * m * n, ARRAY_ZERO);
    FLOAT_TYPE *tempA = createHostArrayFlt(m * n, ARRAY_ZERO);
    FLOAT_TYPE *tempB = createHostArrayFlt(m * n, ARRAY_ZERO);
    ...
    FLOAT_TYPE *f_prev = createHostArrayFlt(9 * m * n, ARRAY_COPY,0,r_f);
	...
	void *hostArrays[] = { nodeIdX, nodeIdY, nodeX, nodeY, nodeType, bcNodeIdX,
			...
			u0, v0, drag, lift, f, fColl, temp9a, temp9b, tempA, tempB, bcMask,
			bcMaskCollapsed, bcIdx, bcIdxCollapsed, qCollapsed, p_in, p_out, num_in, num_out};
	...
	while (iter < args->iterations) {
...
#else
			//			cpuStreaming2D(nodeType, stream_d, r_f_d, r_fColl_d);
			//			cpuStreaming2D(nodeType, stream_d, b_f_d, b_fColl_d);
			cpuStreaming2DCG(nodeType, stream, r_f, r_fColl, b_f, b_fColl, cg_directions);

#endif
		}
		else{
			cpuStreaming2D(nodeType, stream, f, fColl);
		}
		/*CHECK(cudaEventRecord(stop, 0));
		CHECK(cudaEventSynchronize(stop));
		CHECK(cudaEventElapsedTime(&cudatime, start, stop));
		...
		if(args->multiPhase){
			...
			cpuBcInlet2D(bcIdxCollapsed, bcMaskCollapsed, f,
					u0, v0, bcCount);
			cpuBcWall2D(bcIdxCollapsed, bcMaskCollapsed, f,
					fColl, qCollapsed, bcCount, args->boundaryType);
			cpuBcOutlet2D(bcIdxCollapsed, bcMaskCollapsed, f,
					u0, v0, bcCount, args->outletProfile);
		}
		...
		if (AuxMacroDiff * args->ShowMacroDiff == iter + 1) {
			...
			if(args->multiPhase){
				...
				f_prev = createHostArrayFlt(n * m * 9, ARRAY_CPYD, 0, f);

			}else{
				r = computeResidual2D(f, fColl, temp9a, temp9b, m,n);
				if (r != r) {
					fprintf(stderr, "\nDIVERGENCE!\n");
				...
				if (args->boundaryId > 0) {
					dragSum[iter] = computeDragLift2D(bcMask, drag, tempA,
							tempB, m, n, args->boundaryId);
					liftSum[iter] = computeDragLift2D(bcMask, lift, tempA,
							tempB, m, n, args->boundaryId);
				}
			}
