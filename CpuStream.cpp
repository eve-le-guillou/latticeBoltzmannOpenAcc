...
void cpuStreaming2D(int* fluid_d, int* stream_d, FLOAT_TYPE* f_d, FLOAT_TYPE* fColl_d)
{
    ...
    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            ...
            f = f_d +
                ms;                // f is f_d memory positions but f starts in f_d 1st level==1st lattice direction
            mf = fColl_d + ms;
            ...
            f[ind + 2 * ms] = (stream_d[ind + 2 * ms] == 1) ? mf[ind + 2 * ms + 1] : mf[ind +
                                                                                        2 * ms];  // "the regular case"
            f[ind + 3 * ms] = (stream_d[ind + 3 * ms] == 1) ? mf[ind + 3 * ms + n] : mf[ind +
                                                                                        3 * ms];  // stream_d != 1 means
            f[ind + 4 * ms] = (stream_d[ind + 4 * ms] == 1) ? mf[ind + 4 * ms - n - 1] : mf[ind + 4 *
                                                                                                  ms];    // wall or node outside dom.
