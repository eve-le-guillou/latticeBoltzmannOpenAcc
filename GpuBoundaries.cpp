...
void gpuBcInlet3D(int *bcIdx_d, unsigned long long *bcMask_d,
	...
	#pragma acc parallel loop present(f_d[0:ms*19], u1_d[0:ms], v1_d[0:ms], w1_d[0:ms], bcMask_d[0:size], bcIdx_d[0:size]) num_gangs(64) vector_length(32) async
...
void gpuBcSimpleWall3D(int *bcIdx_d, unsigned long long *bcMask_d,
	...
	int ms = depth_d * length_d * height_d;
	int dir;
        #pragma acc parallel loop present(f_d[0:ms*19], bcMask_d[0:size], bcIdx_d[0:size], fColl_d[0:ms*19], q_d[0:18*size]) num_gangs(GANGS) vector_length(THREADS) async 
	for (int bci = 0; bci < size; bci++) {
		int ind = bcIdx_d[bci];
		if (bcMask_d[bci] & BC3D_FLUID) {
...
void gpuBcPeriodic3D(int *bcIdx_d, unsigned long long *bcMask_d,
	...
	#pragma acc parallel loop present(bcMask_d[0:size], f_d[0:19*ms], bcIdx_d[0:size]) firstprivate(offsetX, offsetY, offsetZ) num_gangs(1024) vector_length(128) async 
	for (int bci = 0; bci < size; bci++) {
		int ind = bcIdx_d[bci];
		if (bcMask_d[bci] & BC3D_FLUID) {
