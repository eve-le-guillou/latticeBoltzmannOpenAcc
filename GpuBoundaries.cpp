...
void gpuBcInlet3D(int *bcIdx_d, unsigned long long *bcMask_d,
	...
	#pragma acc parallel loop copy(f_d[0:ms*19], u1_d[0:ms], v1_d[0:ms], w1_d[0:ms], bcMask_d[0:size], bcIdx_d[0:size]) 
...
void gpuBcPeriodic3D(int *bcIdx_d, unsigned long long *bcMask_d,
	...
	//#pragma acc parallel loop copy(bcMask_d[0:size], f_d[0:19*ms], bcIdx_d[0:size]) firstprivate(offsetX, offsetY, offsetZ)
	for (int bci = 0; bci < size; bci++) {
		int ind = bcIdx_d[bci];
		if (bcMask_d[bci] & BC3D_FLUID) {
