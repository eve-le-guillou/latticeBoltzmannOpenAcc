/**
 * Collision model
 * @file CpuCollision.cu
 * @author Adam Koleszar (adam.koleszar@gmail.com)
 */

#include "CpuFunctions.h"
#include "CpuConstants.h"

/**
 * @brief Compute the equilibrum distribution without the collision frequency
 * @details ...
 * @todo doc: insert name or reference, explain method
 *
 * @param u,v velocity
 * @param uc,vc velocity component, see: #cx_d #cy_d
 * @param rho density
 * @param w lattice weight, see: #w_d
 * @return equlibrum distribution function
 */
FLOAT_TYPE feqc2D(FLOAT_TYPE u, FLOAT_TYPE uc, FLOAT_TYPE v, FLOAT_TYPE vc, FLOAT_TYPE rho, FLOAT_TYPE w)
{
    FLOAT_TYPE vec = u*uc + v*vc;
    return rho * w * (1. + 3.*vec + 4.5*vec*vec - 1.5*(u*u + v*v));
}

FLOAT_TYPE feqc3D(FLOAT_TYPE u, FLOAT_TYPE uc, FLOAT_TYPE v, FLOAT_TYPE vc, FLOAT_TYPE w, FLOAT_TYPE wc, FLOAT_TYPE rho, FLOAT_TYPE weight)
{
    FLOAT_TYPE vec = u*uc + v*vc + w*wc;
    return rho * weight * (1. + 3.*vec + 4.5*vec*vec - 1.5*(u*u + v*v+ w*w));
}

FLOAT_TYPE calculateColorGradientY(FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d,
int cg_dir_d, int index){

    FLOAT_TYPE res = 0.0;
    int n = length;
    FLOAT_TYPE cg_w1 = 4.0/12.0;
    FLOAT_TYPE cg_w2 = 1.0/12.0;
    switch (cg_dir_d) {
        case 0:
            res += (r_rho_d[index + n] - b_rho_d[index + n])  *  cg_w1;
            res += (r_rho_d[index - n] - b_rho_d[index - n])  * -cg_w1;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])  *  cg_w2;
            res += (r_rho_d[index + n - 1] - b_rho_d[index + n - 1]) * cg_w2;
            res += (r_rho_d[index - n - 1] - b_rho_d[index - n - 1])   * -cg_w2;
            res += (r_rho_d[index - n + 1] - b_rho_d[index - n + 1])  *  -cg_w2;
            break;
        case 3: //EAST
            res += (r_rho_d[index + n] - b_rho_d[index + n])  *  cg_w1;
            res += (r_rho_d[index - n] - b_rho_d[index - n])  * -cg_w1;
            res += (r_rho_d[index + n - 1] - b_rho_d[index + n - 1])  *  cg_w2;
            res += (r_rho_d[index + n - 1] - b_rho_d[index + n - 1]) * cg_w2;
            res += (r_rho_d[index - n - 1] - b_rho_d[index - n - 1])   * -cg_w2;
            res += (r_rho_d[index - n - 1] - b_rho_d[index - n - 1])  *  -cg_w2;
            break;
        case 4: //WEST
            res += (r_rho_d[index + 1] - b_rho_d[index + 1])  *  cg_w1;
            res += (r_rho_d[index - 1] - b_rho_d[index - 1])  * -cg_w1;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])  *  cg_w2;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1]) * -cg_w2;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])   * -cg_w2;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])  *  cg_w2;
            break;
        default:
            break;
    }

    return res;
}

FLOAT_TYPE calculateColorGradientX(FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d,	int cg_dir_d, int index){

    FLOAT_TYPE res = 0.0;
    int n = length;

    FLOAT_TYPE cg_w1 = 4.0/12.0;
    FLOAT_TYPE cg_w2 = 1.0/12.0;
    switch (cg_dir_d) {
        case 0:
            res += (r_rho_d[index + 1] - b_rho_d[index + 1])  * cg_w1;
            res += (r_rho_d[index - 1] - b_rho_d[index - 1])  * -cg_w1;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])  *  cg_w2;
            res += (r_rho_d[index + n - 1] - b_rho_d[index + n - 1]) * -cg_w2;
            res += (r_rho_d[index - n - 1] - b_rho_d[index - n - 1])   * -cg_w2;
            res += (r_rho_d[index - n + 1] - b_rho_d[index - n + 1])  *  cg_w2;
            break;
        case 1: //NORTH
            res += (r_rho_d[index + 1] - b_rho_d[index + 1])  *  cg_w1;
            res += (r_rho_d[index - 1] - b_rho_d[index - 1])  * -cg_w1;
            res += (r_rho_d[index - n + 1] - b_rho_d[index - n + 1])  *  cg_w2;
            res += (r_rho_d[index - n - 1] - b_rho_d[index - n - 1]) * -cg_w2;
            res += (r_rho_d[index - n - 1] - b_rho_d[index - n - 1])   * -cg_w2;
            res += (r_rho_d[index - n + 1] - b_rho_d[index - n + 1])  *  cg_w2;
            break;
        case 2: //SOUTH
            res += (r_rho_d[index + 1] - b_rho_d[index + 1])  *  cg_w1;
            res += (r_rho_d[index - 1] - b_rho_d[index - 1])  * -cg_w1;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])  *  cg_w2;
            res += (r_rho_d[index + n - 1] - b_rho_d[index + n - 1]) * -cg_w2;
            res += (r_rho_d[index + n - 1] - b_rho_d[index + n - 1])   * -cg_w2;
            res += (r_rho_d[index + n + 1] - b_rho_d[index + n + 1])  *  cg_w2;
            break;
        default:
            break;
    }
    return res;
}

void calculateColorGradient(FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d,FLOAT_TYPE *rho_d, int cg_dir_d,
    int index, FLOAT_TYPE *cg_x, FLOAT_TYPE *cg_y, FLOAT_TYPE *gr_x, FLOAT_TYPE *gr_y){
    FLOAT_TYPE cgx = 0.0;
    FLOAT_TYPE cgy = 0.0;
    FLOAT_TYPE grx = 0.0;
    FLOAT_TYPE gry = 0.0;
    FLOAT_TYPE  aux1, aux2;
    int ind, i;
    switch (cg_dir_d) {
        case 0:
            #pragma unroll 8
            for(i = 1; i < 9; i++){
                ind = index + cx2D[i] + cy2D[i] * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                grx += aux2 * cx2D[i];
                gry += aux2 * cy2D[i];

                cgx += aux1 * cx2D[i];
                cgy += aux1 * cy2D[i];
            }
            break;
        case 1: //NORTH
            #pragma unroll 8
            for(i = 1; i < 9; i++){
                ind = index + cx2D[i] - abs(cy2D[i]) * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];
                grx += aux2 * cx2D[i];
                cgx += aux1 * cx2D[i];
            }
            break;
        case 2: //SOUTH
            #pragma unroll 8
            for(i = 1; i < 9; i++){
                ind = index + cx2D[i] + abs(cy2D[i]) * length;

                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                grx += aux2 * cx2D[i];
                cgx += aux1 * cx2D[i];
            }
            break;
        case 3: //EAST
            #pragma unroll 8
            for(i = 1; i < 9; i++){
                ind = index - abs(cx2D[i]) + cy2D[i] * length;

                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                gry += aux2 * cy2D[i];

                cgy += aux1 * cy2D[i];
            }
            break;
        case 4: //WEST
            #pragma unroll 8
            for(i = 1; i < 9; i++){
                ind = index + abs(cx2D[i]) + cy2D[i] * length;

                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                gry += aux2 * cy2D[i];

                cgy += aux1 * cy2D[i];
            }
            break;
        default:
            break;
    }
    (*cg_x) = cgx;
    (*cg_y) = cgy;

    (*gr_x) = grx;
    (*gr_y) = gry;
}

void calculateHOColorGradient(FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, FLOAT_TYPE *rho_d, int cg_dir_d, int index, FLOAT_TYPE *cg_x, FLOAT_TYPE *cg_y,
        FLOAT_TYPE *gr_x, FLOAT_TYPE *gr_y){
    FLOAT_TYPE cgx = 0.0;
    FLOAT_TYPE cgy = 0.0;
    FLOAT_TYPE grx = 0.0;
    FLOAT_TYPE gry = 0.0;
    FLOAT_TYPE aux1, aux2;
    int ind, i;
    switch (cg_dir_d) {
        case 0:
            for(i = 1; i < 25; i++){
                ind = index + hocg_cx[i] + hocg_cy[i] * length;
                aux1 = hocg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = hocg_w[i] * rho_d[ind];

                grx += aux2 * hocg_cx[i];
                gry += aux2 * hocg_cy[i];

                cgx += aux1 * hocg_cx[i];
                cgy += aux1 * hocg_cy[i];
            }
            break;
        case 1: //NORTH
            for(i = 1; i < 9; i++){
                ind = index + cx2D[i] - abs(cy2D[i]) * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                grx += aux2 * cx2D[i];

                cgx += aux1 * cx2D[i];
            }
            break;
        case 2: //SOUTH
            for(i = 1; i < 9; i++){
                ind = index + cx2D[i] + abs(cy2D[i]) * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                grx += aux2 * cx2D[i];

                cgx += aux1 * cx2D[i];
            }
            break;
        case 3: //EAST
            for(i = 1; i < 9; i++){
                ind = index - abs(cx2D[i]) + cy2D[i] * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                gry += aux2 * cy2D[i];

                cgy += aux1 * cy2D[i];
            }
            break;
        case 4: //WEST
            for(i = 1; i < 9; i++){
                ind = index + abs(cx2D[i]) + cy2D[i] * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                gry += aux2 * cy2D[i];

                cgy += aux1 * cy2D[i];
            }
            break;
        case 11:
            for(i = 1; i < 9; i++){
                ind = index + cx2D[i] + cy2D[i] * length;
                aux1 = cg_w[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w[i] * rho_d[ind];

                grx += aux2 * cx2D[i];
                gry += aux2 * cy2D[i];

                cgx += aux1 * cx2D[i];
                cgy += aux1 * cy2D[i];
            }
            break;
        default:
            break;
    }
    (*cg_x) = cgx;
    (*cg_y) = cgy;
    (*gr_x) = grx;
    (*gr_y) = gry;
}

void calculateColorGradient3D(FLOAT_TYPE *rho_d, FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, int cg_dir_d, int index,
        FLOAT_TYPE *cg_x, FLOAT_TYPE *cg_y, FLOAT_TYPE *cg_z, FLOAT_TYPE *gr_x, FLOAT_TYPE *gr_y, FLOAT_TYPE *gr_z){
    FLOAT_TYPE cgx, cgy, cgz, grx,gry,grz;
    FLOAT_TYPE aux1, aux2;
    cgx = cgy = cgz = grx = gry = grz = 0.0;
    int ind, i, ms = length * depth;
    switch (cg_dir_d) {
        case 0:
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index - c3D[i];
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                gry += aux2 * cy3D[i];
                grz += aux2 * cz3D[i];

                cgx += aux1 * cx3D[i];
                cgy += aux1 * cy3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 1: //NORTH
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] - abs(cy3D[i]) * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                grz += aux2 * cz3D[i];

                cgx += aux1 * cx3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 2: //SOUTH
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] + abs(cy3D[i]) * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                grz += aux2 * cz3D[i];

                cgx += aux1 * cx3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 3: //EAST
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index - abs(cx3D[i]) + cy3D[i] * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                gry += aux2 * cy3D[i];
                grz += aux2 * cz3D[i];

                cgy += aux1 * cy3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 4: //WEST
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index + abs(cx3D[i]) + cy3D[i] * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                gry += aux2 * cy3D[i];
                grz += aux2 * cz3D[i];

                cgy += aux1 * cy3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 5: // FRONT
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] + cy3D[i] * length - abs(cz3D[i]) * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                gry += aux2 * cy3D[i];

                cgx += aux1 * cx3D[i];
                cgy += aux1 * cy3D[i];
            }
            break;
        case 6: // BACK
            #pragma unroll 18
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] + cy3D[i] * length + abs(cz3D[i]) * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                gry += aux2 * cy3D[i];

                cgx += aux1 * cx3D[i];
                cgy += aux1 * cy3D[i];
            }
            break;
        default:
            break;
    }
    (*cg_x) = cgx;
    (*cg_y) = cgy;
    (*cg_z) = cgz;
    (*gr_x) = grx;
    (*gr_y) = gry;
    (*gr_z) = grz;
}

void calculateHOColorGradient3D(FLOAT_TYPE *rho_d, FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, int cg_dir_d, int index,
        FLOAT_TYPE *cg_x, FLOAT_TYPE *cg_y, FLOAT_TYPE *cg_z, FLOAT_TYPE *gr_x, FLOAT_TYPE *gr_y, FLOAT_TYPE *gr_z){

    FLOAT_TYPE cgx, cgy, cgz, grx,gry,grz;
    FLOAT_TYPE aux1, aux2;
    cgx = cgy = cgz = grx = gry = grz = 0.0;
    int ind, i, ms = length * depth;
    switch (cg_dir_d) {
        case 0:
            for(i = 1; i < 105; i++){
                ind = index + hoc3D[i];
                aux1 = hocg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = hocg_w3D[i] * rho_d[ind];

                grx += aux2 * hocg_cx3D[i];
                gry += aux2 * hocg_cy3D[i];
                grz += aux2 * hocg_cz3D[i];

                cgx += aux1 * hocg_cx3D[i];
                cgy += aux1 * hocg_cy3D[i];
                cgz += aux1 * hocg_cz3D[i];
            }
            break;
        case 1: //NORTH
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] - abs(cy3D[i]) * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                grz += aux2 * cz3D[i];

                cgx += aux1 * cx3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 2: //SOUTH
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] + abs(cy3D[i]) * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                grz += aux2 * cz3D[i];

                cgx += aux1 * cx3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 3: //EAST
        for(i = 1; i < 19; i++){
                ind = index - abs(cx3D[i]) + cy3D[i] * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                gry += aux2 * cy3D[i];
                grz += aux2 * cz3D[i];

                cgy += aux1 * cy3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 4: //WEST
            for(i = 1; i < 19; i++){
                ind = index + abs(cx3D[i]) + cy3D[i] * length + cz3D[i] * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                gry += aux2 * cy3D[i];
                grz += aux2 * cz3D[i];

                cgy += aux1 * cy3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        case 5: // FRONT
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] + cy3D[i] * length - abs(cz3D[i]) * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                gry += aux2 * cy3D[i];

                cgx += aux1 * cx3D[i];
                cgy += aux1 * cy3D[i];
            }
            break;
        case 6: // BACK
            for(i = 1; i < 19; i++){
                ind = index + cx3D[i] + cy3D[i] * length + abs(cz3D[i]) * ms;
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                gry += aux2 * cy3D[i];

                cgx += aux1 * cx3D[i];
                cgy += aux1 * cy3D[i];
            }
            break;
        case 11:
            for(i = 1; i < 19; i++){
                ind = index - c3D[i];
                aux1 = cg_w3D[i] * (r_rho_d[ind] - b_rho_d[ind]) / rho_d[ind];
                aux2 = cg_w3D[i] * rho_d[ind];

                grx += aux2 * cx3D[i];
                gry += aux2 * cy3D[i];
                grz += aux2 * cz3D[i];

                cgx += aux1 * cx3D[i];
                cgy += aux1 * cy3D[i];
                cgz += aux1 * cz3D[i];
            }
            break;
        default:
            break;
    }
    (*cg_x) = cgx;
    (*cg_y) = cgy;
    (*cg_z) = cgz;
    (*gr_x) = grx;
    (*gr_y) = gry;
    (*gr_z) = grz;
}

FLOAT_TYPE calcPerturb2D(FLOAT_TYPE cg_norm, FLOAT_TYPE w, FLOAT_TYPE prod_c_g, FLOAT_TYPE w_pert){
    if(cg_norm > g_limit)
        return 0.5 * A * cg_norm * (w * (prod_c_g *prod_c_g) / (cg_norm * cg_norm) - w_pert);
    else
        return 0.0;
}

FLOAT_TYPE calcCosin2D(FLOAT_TYPE cg_norm, FLOAT_TYPE c_norm, FLOAT_TYPE prod_c_g){
    if(cg_norm > g_limit)
        return prod_c_g / (cg_norm * c_norm);
    else
        return 0.0;
}

FLOAT_TYPE calcTC(int cx, int cy, FLOAT_TYPE G1, FLOAT_TYPE G2,FLOAT_TYPE G3,FLOAT_TYPE G4){
    return G1 * cx * cx + G2 * cx * cy + G3 * cx * cy + G4 * cy * cy;
}

FLOAT_TYPE feq2DCG(FLOAT_TYPE u, FLOAT_TYPE v, int cx, int cy, FLOAT_TYPE chi, FLOAT_TYPE psi, FLOAT_TYPE teta, FLOAT_TYPE phi,
        FLOAT_TYPE TC, FLOAT_TYPE prod_u_grad_rho, FLOAT_TYPE r, FLOAT_TYPE mean_alpha, FLOAT_TYPE w, FLOAT_TYPE cu1, FLOAT_TYPE mean_nu){
    FLOAT_TYPE cu2 = u*cx + v*cy;
    return mean_nu * (chi * prod_u_grad_rho + psi * TC) + r *
    (phi + teta * mean_alpha + w * (3. * cu2 + 4.5 * cu2 * cu2 - 1.5 * cu1));
}

void cpuCollBgkw2D(int *fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *u_d,
                              FLOAT_TYPE *v_d, FLOAT_TYPE *f_d, FLOAT_TYPE *fColl_d)
{
    int ind = blockIdx.x * blockDim.x + threadIdx.x;

    int ms = depth*length;
    FLOAT_TYPE r, u, v;
    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            u = u_d[ind];
            v = v_d[ind];
            r = rho_d[ind];
            fColl_d[ind] = omega * feqc2D(u, 0, v, 0, r, 4. / 9.) + (1.0 - omega) * f_d[ind];
            fColl_d[ind + 1 * ms] = omega * feqc2D(u, 1, v, 0, r, 1. / 9.) + (1.0 - omega) * f_d[ind + 1 * ms];
            fColl_d[ind + 2 * ms] = omega * feqc2D(u, 0, v, 1, r, 1. / 9.) + (1.0 - omega) * f_d[ind + 2 * ms];
            fColl_d[ind + 3 * ms] = omega * feqc2D(u, -1, v, 0, r, 1. / 9.) + (1.0 - omega) * f_d[ind + 3 * ms];
            fColl_d[ind + 4 * ms] = omega * feqc2D(u, 0, v, -1, r, 1. / 9.) + (1.0 - omega) * f_d[ind + 4 * ms];
            fColl_d[ind + 5 * ms] = omega * feqc2D(u, 1, v, 1, r, 1. / 36.) + (1.0 - omega) * f_d[ind + 5 * ms];
            fColl_d[ind + 6 * ms] = omega * feqc2D(u, -1, v, 1, r, 1. / 36.) + (1.0 - omega) * f_d[ind + 6 * ms];
            fColl_d[ind + 7 * ms] = omega * feqc2D(u, -1, v, -1, r, 1. / 36.) + (1.0 - omega) * f_d[ind + 7 * ms];
            fColl_d[ind + 8 * ms] = omega * feqc2D(u, 1, v, -1, r, 1. / 36.) + (1.0 - omega) * f_d[ind + 8 * ms];
        }
    }
}

void cpuCollBgkwGC2D(FLOAT_TYPE *rho_d, FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, FLOAT_TYPE *u_d,
                                FLOAT_TYPE *v_d, FLOAT_TYPE *f_d, FLOAT_TYPE *r_fColl_d, FLOAT_TYPE *b_fColl_d, int *cg_dir_d, bool high_order){

    int ms = depth*length;
    int cx, cy;
    FLOAT_TYPE r_r, b_r, r, u, v, cg_x, cg_y, gr_x, gr_y;
    FLOAT_TYPE k_r, k_b, k_k, color_gradient_norm, cosin, mean_nu, omega_eff;
    FLOAT_TYPE prod_c_g, pert;
    FLOAT_TYPE f_CollPert;
    FLOAT_TYPE mean_alpha, cu1, cu2, f_eq;
    for (int ind = 0; ind < ms; ind++){
        u =   u_d[ind];
        v =   v_d[ind];
        r_r = r_rho_d[ind];
        b_r = b_rho_d[ind];
        r = rho_d[ind];

        if(high_order)
            calculateHOColorGradient(r_rho_d,b_rho_d, rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &gr_x, &gr_y);
        else{
            calculateColorGradient(r_rho_d,b_rho_d, rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &gr_x, &gr_y);
        }

        mean_nu = 1.0 / (r_r / (r * r_viscosity) + b_r /(r * b_viscosity));
        omega_eff = 1.0 / (3.0 * mean_nu + 0.5);
        mean_alpha = r_alpha * r_r / r + b_alpha * b_r / r;

        color_gradient_norm = sqrt(cg_x * cg_x + cg_y * cg_y);
        k_r = r_r / r;
        k_b = b_r / r;
        k_k = beta * r_r * b_r / r;

        cu1 = u*u + v*v;

        #pragma unroll 9
        for (int k=0;k<9;k++){
            cx = cx2D[k];
            cy = cy2D[k];
            if (color_gradient_norm > g_limit){
                prod_c_g=cx * cg_x + cy * cg_y;
                if (k!=0){
                    cosin= prod_c_g / (color_gradient_norm*c_norms[k]);
                }
                else
                    cosin=0.0;

                // calculate perturbation terms
                pert = A * color_gradient_norm * (w2D[k]* (prod_c_g *prod_c_g) / (color_gradient_norm * color_gradient_norm) - w_pert[k]);
                }
            else{
                // the perturbation terms are null
                pert=0.0;
                cosin=0.0;
            }

            cu2 = u*cx + v*cy;
            f_eq = r * (phi[k] + teta[k] * mean_alpha + w2D[k] * (3. * cu2 + 4.5 * cu2 * cu2 - 1.5 * cu1));

            // calculate updated distribution function
            f_CollPert = omega_eff * f_eq + (1 - omega_eff) * f_d[ind + k * ms] + pert;

            r_fColl_d[ind + k * ms] = k_r * f_CollPert + k_k * cosin * (phi[k] + teta[k] * mean_alpha);
            b_fColl_d[ind + k * ms] = k_b * f_CollPert - k_k * cosin * (phi[k] + teta[k] * mean_alpha);

        }
    }
}

void cpuCollEnhancedBgkwGC2D(FLOAT_TYPE *rho_d, FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, FLOAT_TYPE *u_d,
                                        FLOAT_TYPE *v_d, FLOAT_TYPE *f_d, FLOAT_TYPE *r_fColl_d, FLOAT_TYPE *b_fColl_d, int *cg_dir_d, bool high_order){

    int ind = blockIdx.x * blockDim.x + threadIdx.x;
    int ms = depth*length;
    int cx, cy;
    FLOAT_TYPE r_r, b_r, r, u, v, cg_x, cg_y, gr_x, gr_y;
    FLOAT_TYPE k_r, k_b, k_k, color_gradient_norm, cosin, mean_nu, omega_eff;
    FLOAT_TYPE prod_c_g, pert;
    FLOAT_TYPE f_CollPert;
    FLOAT_TYPE G1, G2, G3, G4, prod_u_grad_rho, mean_alpha, TC, cu1, cu2, f_eq;
    for (int ind = 0; ind < ms; ind++){
        u =   u_d[ind];
        v =   v_d[ind];
        r_r = r_rho_d[ind];
        b_r = b_rho_d[ind];
        r = rho_d[ind];


        if(high_order)
            calculateHOColorGradient(r_rho_d,b_rho_d, rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &gr_x, &gr_y);
        else{
            calculateColorGradient(r_rho_d,b_rho_d, rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &gr_x, &gr_y);
        }

        G1 = 2.0 * u * gr_x;
        G2 = u * gr_y + v*gr_x;
        G3 = G2;
        G4 = 2.0 * v * gr_y;

        prod_u_grad_rho = u * gr_x + v * gr_y;

        mean_nu = 1.0 / (r_r / (r * r_viscosity) + b_r /(r * b_viscosity));
        omega_eff = 1.0 / (3.0 * mean_nu + 0.5);
        mean_alpha = r_alpha * r_r / r + b_alpha * b_r / r;


        color_gradient_norm = sqrt(cg_x * cg_x + cg_y * cg_y);
        k_r=r_r/r;
        k_b=b_r/r;
        k_k= beta * r_r * b_r / r;

        cu1 = u*u + v*v;

        #pragma unroll 9
        for (int k=0;k<9;k++){
            cx = cx2D[k];
            cy = cy2D[k];
            if (color_gradient_norm > g_limit){
                prod_c_g=cx * cg_x + cy * cg_y;
                if (k!=0){
                    cosin= prod_c_g / (color_gradient_norm*c_norms[k]);
                }
                else
                    cosin=0.0;

                // calculate perturbation terms
                pert= A * color_gradient_norm * (w2D[k]* (prod_c_g *prod_c_g) / (color_gradient_norm * color_gradient_norm) - w_pert[k]);
            }
            else{
                // the perturbation terms are null
                pert=0.0;
            }

            TC = 0.0;
            TC += G1 * cx * cx;
            TC += G2 * cx * cy;
            TC += G3 * cx * cy;
            TC += G4 * cy * cy;

            cu2 = u*cx + v*cy;
            f_eq = mean_nu * (chi[k] * prod_u_grad_rho + psi[k] * TC) + r *
                                                                            (phi[k] + teta[k] * mean_alpha + w2D[k] * (3. * cu2 + 4.5 * cu2 * cu2 - 1.5 * cu1));

            // calculate updated distribution function
            f_CollPert = omega_eff*f_eq + (1-omega_eff) * f_d[ind + k * ms] + pert;

            r_fColl_d[ind + k * ms] = k_r * f_CollPert + k_k * cosin * (phi[k] + teta[k] * mean_alpha);
            b_fColl_d[ind + k * ms] = k_b * f_CollPert - k_k * cosin * (phi[k] + teta[k] * mean_alpha);
        }
    }
}

void cpuCollBgkwGC3D(int *fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, FLOAT_TYPE *u_d,
                                FLOAT_TYPE *v_d, FLOAT_TYPE *w_d, FLOAT_TYPE *f_d, FLOAT_TYPE *r_fColl_d, FLOAT_TYPE *b_fColl_d, int *cg_dir_d, bool high_order){

    int ms = depth*length*height;
    FLOAT_TYPE r_r, b_r, r, u, v, w, cg_x, cg_y, cg_z, gr_x, gr_y, gr_z;
    FLOAT_TYPE k_r, k_b, k_k, color_gradient_norm, cosin, mean_nu, omega_eff, mean_alpha;
    FLOAT_TYPE prod_c_g, pert, cu1, cu2;
    FLOAT_TYPE f_CollPert, f_eq;
    int cx, cy, cz;
    for (int ind = 0; ind < ms; ind++){
        u =   u_d[ind];
        v =   v_d[ind];
        w =   w_d[ind];
        r_r = r_rho_d[ind];
        b_r = b_rho_d[ind];
        r = rho_d[ind];

        if(high_order)
            calculateHOColorGradient3D(rho_d, r_rho_d, b_rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &cg_z, &gr_x, &gr_y, &gr_z);
        else
            calculateColorGradient3D(rho_d, r_rho_d, b_rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &cg_z, &gr_x, &gr_y, &gr_z);

        cu1 = u*u + v*v + w * w;

        // invariable quantities
        color_gradient_norm = sqrt(cg_x * cg_x + cg_y * cg_y + cg_z * cg_z);

        k_r = r_r / r;
        k_b = b_r / r;
        k_k = beta * r_r * b_r / r;

        mean_nu = 1.0 / (r_r / (r * r_viscosity ) + b_r / (r * b_viscosity));
        omega_eff = 1.0/(3.0*mean_nu+0.5);
        mean_alpha = r_alpha * r_r / r + b_alpha * b_r / r;

        #pragma unroll 19
        for (int dir=0;dir < 19;dir++){
            cx = cx3D[dir];
            cy = cy3D[dir];
            cz = cz3D[dir];
            if (color_gradient_norm > g_limit){
                prod_c_g = cx * cg_x + cy * cg_y + cz * cg_z;
                if (dir != 0){
                    cosin = prod_c_g / (color_gradient_norm * c_norms3D[dir]);
                }
                else
                    cosin=0.0;
                // calculate perturbation terms
                pert = A * color_gradient_norm * (w3D[dir]* (prod_c_g *prod_c_g) / (color_gradient_norm * color_gradient_norm) - w_pert3D[dir]);
            }
            else{
                // the perturbation terms are null
                pert = 0.0;
            }

            cu2 = u*cx + v*cy + w*cz;
            // calculate equilibrium distribution function
            f_eq = r * (phi3D[dir] + teta3D[dir] * mean_alpha + w3D[dir] * (3. * cu2 + 4.5 * cu2 * cu2 - 1.5 * cu1));

            // calculate updated distribution function
            f_CollPert = omega_eff*f_eq + (1-omega_eff)*f_d[ind + dir * ms] + pert;


            r_fColl_d[ind + dir * ms] = k_r * f_CollPert + k_k * cosin * (phi3D[dir] + teta3D[dir] * mean_alpha);
            b_fColl_d[ind + dir * ms] = k_b * f_CollPert - k_k * cosin * (phi3D[dir] + teta3D[dir] * mean_alpha);
        }
    }

}

void cpuCollEnhancedBgkwGC3D(int *fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *r_rho_d, FLOAT_TYPE *b_rho_d, FLOAT_TYPE *u_d,
                                        FLOAT_TYPE *v_d, FLOAT_TYPE *w_d, FLOAT_TYPE *f_d, FLOAT_TYPE *r_fColl_d, FLOAT_TYPE *b_fColl_d, int *cg_dir_d, bool high_order){

    int ms = depth*length*height;
    FLOAT_TYPE r_r, b_r, r, u, v, w, cg_x, cg_y, cg_z, gr_x, gr_y, gr_z;
    FLOAT_TYPE k_r, k_b, k_k, color_gradient_norm, cosin, mean_nu, omega_eff, TC, mean_alpha;
    FLOAT_TYPE prod_c_g, pert, prod_u_grad_rho, cu1, cu2;
    FLOAT_TYPE f_CollPert, f_eq;
    FLOAT_TYPE G1, G2, G3, G4, G5, G6, G7, G8, G9;
    int cx, cy, cz;
    for (int ind = 0; ind < ms; ind++){
        u =   u_d[ind];
        v =   v_d[ind];
        w =   w_d[ind];
        r_r = r_rho_d[ind];
        b_r = b_rho_d[ind];
        r = rho_d[ind];

        if(high_order)
            calculateHOColorGradient3D(rho_d, r_rho_d, b_rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &cg_z, &gr_x, &gr_y, &gr_z);
        else
            calculateColorGradient3D(rho_d, r_rho_d, b_rho_d, cg_dir_d[ind], ind, &cg_x, &cg_y, &cg_z, &gr_x, &gr_y, &gr_z);

        G1 = 2.0 * u * gr_x;
        G2 = u * gr_y + v*gr_x;
        G3 = u * gr_z + w*gr_x;
        G4 = G2;
        G5 = 2.0 * v * gr_y;
        G6 = v * gr_z + w * gr_y;
        G7 = G3;
        G8 = G6;
        G9 = 2.0 * w * gr_z;

        prod_u_grad_rho = u * gr_x + v * gr_y + w * gr_z;

        cu1 = u*u + v*v + w * w;

        // invariable quantities
        color_gradient_norm = sqrt(cg_x * cg_x + cg_y * cg_y + cg_z * cg_z);

        k_r = r_r / r;
        k_b = b_r / r;
        k_k = beta * r_r * b_r / r;

        mean_nu = 1.0 / (r_r / (r * r_viscosity ) + b_r / (r * b_viscosity));
        omega_eff = 1.0/(3.0*mean_nu+0.5);
        mean_alpha = r_alpha * r_r / r + b_alpha * b_r / r;

        for (int dir=0;dir < 19;dir++){
            cx = cx3D[dir];
            cy = cy3D[dir];
            cz = cz3D[dir];
            if (color_gradient_norm > g_limit){
                prod_c_g = cx * cg_x + cy * cg_y + cz * cg_z;
                if (dir != 0){
                    cosin = prod_c_g / (color_gradient_norm * c_norms3D[dir]);
                }
                else
                    cosin=0.0;
                // calculate perturbation terms
                pert= A * color_gradient_norm * (w3D[dir]* (prod_c_g *prod_c_g) / (color_gradient_norm * color_gradient_norm) - w_pert3D[dir]);
            }
            else{
                // the perturbation terms are null
                pert = 0.0;
            }

            // Auxiliar tensor: diadic product of the speed velcity:
            //[cx,cy,cx]*[cx cy cz]
            //Tensor contraction
            TC = 0.0;
            TC += G1 * cx * cx;
            TC += G2 * cx * cy;
            TC += G3 * cx * cz;
            TC += G4 * cx * cy; // H = 1
            TC += G5 * cy * cy;
            TC += G6 * cy * cz;
            TC += G7 * cx * cz; // H = 2
            TC += G8 * cy * cz; // H= 5
            TC += G9 * cz * cz;

            cu2 = u*cx + v*cy + w*cz;
            // calculate equilibrium distribution function
            f_eq = mean_nu * (chi3D[dir] * prod_u_grad_rho + psi3D[dir] * TC) + r *
                                                                                    (phi3D[dir] + teta3D[dir] * mean_alpha + w3D[dir] * (3. * cu2 + 4.5 * cu2 * cu2 - 1.5 * cu1));

            // calculate updated distribution function
            f_CollPert = omega_eff*f_eq + (1-omega_eff)*f_d[ind + dir * ms] + pert;


            r_fColl_d[ind + dir * ms] = k_r * f_CollPert + k_k * cosin * (phi3D[dir] + teta3D[dir] * mean_alpha);
            b_fColl_d[ind + dir * ms] = k_b * f_CollPert - k_k * cosin * (phi3D[dir] + teta3D[dir] * mean_alpha);
        }
    }

}

void cpuCollBgkw3D(int *fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *u_d,
                              FLOAT_TYPE *v_d, FLOAT_TYPE *w_d, FLOAT_TYPE *f_d, FLOAT_TYPE *fColl_d)
{
    int ind =  blockId * (blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x) + threadIdx.x;

    int ms = depth*length*height;
    FLOAT_TYPE r, u, v, w;
    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            u = u_d[ind];
            v = v_d[ind];
            w = w_d[ind];
            r = rho_d[ind];
            fColl_d[ind + 0 * ms] = omega * feqc3D(u, cx3D[0], v, cy3D[0], w, cz3D[0], r, w3D[0]) +
                                    (1.0 - omega) * f_d[ind + 0 * ms];
            fColl_d[ind + 1 * ms] = omega * feqc3D(u, cx3D[1], v, cy3D[1], w, cz3D[1], r, w3D[1]) +
                                    (1.0 - omega) * f_d[ind + 1 * ms];
            fColl_d[ind + 2 * ms] = omega * feqc3D(u, cx3D[2], v, cy3D[2], w, cz3D[2], r, w3D[2]) +
                                    (1.0 - omega) * f_d[ind + 2 * ms];
            fColl_d[ind + 3 * ms] = omega * feqc3D(u, cx3D[3], v, cy3D[3], w, cz3D[3], r, w3D[3]) +
                                    (1.0 - omega) * f_d[ind + 3 * ms];
            fColl_d[ind + 4 * ms] = omega * feqc3D(u, cx3D[4], v, cy3D[4], w, cz3D[4], r, w3D[4]) +
                                    (1.0 - omega) * f_d[ind + 4 * ms];
            fColl_d[ind + 5 * ms] = omega * feqc3D(u, cx3D[5], v, cy3D[5], w, cz3D[5], r, w3D[5]) +
                                    (1.0 - omega) * f_d[ind + 5 * ms];
            fColl_d[ind + 6 * ms] = omega * feqc3D(u, cx3D[6], v, cy3D[6], w, cz3D[6], r, w3D[6]) +
                                    (1.0 - omega) * f_d[ind + 6 * ms];
            fColl_d[ind + 7 * ms] = omega * feqc3D(u, cx3D[7], v, cy3D[7], w, cz3D[7], r, w3D[7]) +
                                    (1.0 - omega) * f_d[ind + 7 * ms];
            fColl_d[ind + 8 * ms] = omega * feqc3D(u, cx3D[8], v, cy3D[8], w, cz3D[8], r, w3D[8]) +
                                    (1.0 - omega) * f_d[ind + 8 * ms];
            fColl_d[ind + 9 * ms] = omega * feqc3D(u, cx3D[9], v, cy3D[9], w, cz3D[9], r, w3D[9]) +
                                    (1.0 - omega) * f_d[ind + 9 * ms];
            fColl_d[ind + 10 * ms] = omega * feqc3D(u, cx3D[10], v, cy3D[10], w, cz3D[10], r, w3D[10]) +
                                     (1.0 - omega) * f_d[ind + 10 * ms];
            fColl_d[ind + 11 * ms] = omega * feqc3D(u, cx3D[11], v, cy3D[11], w, cz3D[11], r, w3D[11]) +
                                     (1.0 - omega) * f_d[ind + 11 * ms];
            fColl_d[ind + 12 * ms] = omega * feqc3D(u, cx3D[12], v, cy3D[12], w, cz3D[12], r, w3D[12]) +
                                     (1.0 - omega) * f_d[ind + 12 * ms];
            fColl_d[ind + 13 * ms] = omega * feqc3D(u, cx3D[13], v, cy3D[13], w, cz3D[13], r, w3D[13]) +
                                     (1.0 - omega) * f_d[ind + 13 * ms];
            fColl_d[ind + 14 * ms] = omega * feqc3D(u, cx3D[14], v, cy3D[14], w, cz3D[14], r, w3D[14]) +
                                     (1.0 - omega) * f_d[ind + 14 * ms];
            fColl_d[ind + 15 * ms] = omega * feqc3D(u, cx3D[15], v, cy3D[15], w, cz3D[15], r, w3D[15]) +
                                     (1.0 - omega) * f_d[ind + 15 * ms];
            fColl_d[ind + 16 * ms] = omega * feqc3D(u, cx3D[16], v, cy3D[16], w, cz3D[16], r, w3D[16]) +
                                     (1.0 - omega) * f_d[ind + 16 * ms];
            fColl_d[ind + 17 * ms] = omega * feqc3D(u, cx3D[17], v, cy3D[17], w, cz3D[17], r, w3D[17]) +
                                     (1.0 - omega) * f_d[ind + 17 * ms];
            fColl_d[ind + 18 * ms] = omega * feqc3D(u, cx3D[18], v, cy3D[18], w, cz3D[18], r, w3D[18]) +
                                     (1.0 - omega) * f_d[ind + 18 * ms];
        }
    }
}
void cpuCollTrt(int *fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *u_d,
                           FLOAT_TYPE *v_d, FLOAT_TYPE *f_d, FLOAT_TYPE *fColl_d)
{
    int ind = blockIdx.x * blockDim.x + threadIdx.x;
    int ms = depth*length;
    FLOAT_TYPE r, u, v;
    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            u = u_d[ind];
            v = v_d[ind];
            r = rho_d[ind];

            FLOAT_TYPE feq0 = feqc2D(u, 0, v, 0, r, 4. / 9.);
            FLOAT_TYPE feq1 = feqc2D(u, 1, v, 0, r, 1. / 9.);
            FLOAT_TYPE feq2 = feqc2D(u, 0, v, 1, r, 1. / 9.);
            FLOAT_TYPE feq3 = feqc2D(u, -1, v, 0, r, 1. / 9.);
            FLOAT_TYPE feq4 = feqc2D(u, 0, v, -1, r, 1. / 9.);
            FLOAT_TYPE feq5 = feqc2D(u, 1, v, 1, r, 1. / 36.);
            FLOAT_TYPE feq6 = feqc2D(u, -1, v, 1, r, 1. / 36.);
            FLOAT_TYPE feq7 = feqc2D(u, -1, v, -1, r, 1. / 36.);
            FLOAT_TYPE feq8 = feqc2D(u, 1, v, -1, r, 1. / 36.);

            FLOAT_TYPE f0 = f_d[ind];
            FLOAT_TYPE f1 = f_d[ind + 1 * ms];
            FLOAT_TYPE f2 = f_d[ind + 2 * ms];
            FLOAT_TYPE f3 = f_d[ind + 3 * ms];
            FLOAT_TYPE f4 = f_d[ind + 4 * ms];
            FLOAT_TYPE f5 = f_d[ind + 5 * ms];
            FLOAT_TYPE f6 = f_d[ind + 6 * ms];
            FLOAT_TYPE f7 = f_d[ind + 7 * ms];
            FLOAT_TYPE f8 = f_d[ind + 8 * ms];

            fColl_d[ind] = f0 - 0.5 * omega * (f0 + f0 - feq0 - feq0) - 0.5 * omegaA * (f0 - f0 - feq0 + feq0);
            fColl_d[ind + 1 * ms] =
                    f1 - 0.5 * omega * (f1 + f3 - feq1 - feq3) - 0.5 * omegaA * (f1 - f3 - feq1 + feq3);
            fColl_d[ind + 2 * ms] =
                    f2 - 0.5 * omega * (f2 + f4 - feq2 - feq4) - 0.5 * omegaA * (f2 - f4 - feq2 + feq4);
            fColl_d[ind + 3 * ms] =
                    f3 - 0.5 * omega * (f3 + f1 - feq3 - feq1) - 0.5 * omegaA * (f3 - f1 - feq3 + feq1);
            fColl_d[ind + 4 * ms] =
                    f4 - 0.5 * omega * (f4 + f2 - feq4 - feq2) - 0.5 * omegaA * (f4 - f2 - feq4 + feq2);
            fColl_d[ind + 5 * ms] =
                    f5 - 0.5 * omega * (f5 + f7 - feq5 - feq7) - 0.5 * omegaA * (f5 - f7 - feq5 + feq7);
            fColl_d[ind + 6 * ms] =
                    f6 - 0.5 * omega * (f6 + f8 - feq6 - feq8) - 0.5 * omegaA * (f6 - f8 - feq6 + feq8);
            fColl_d[ind + 7 * ms] =
                    f7 - 0.5 * omega * (f7 + f5 - feq7 - feq5) - 0.5 * omegaA * (f7 - f5 - feq7 + feq5);
            fColl_d[ind + 8 * ms] =
                    f8 - 0.5 * omega * (f8 + f6 - feq8 - feq6) - 0.5 * omegaA * (f8 - f6 - feq8 + feq6);

            // fColl_d[ind]      = f0 - omega * (0.5*(f0+f0) - 0.5*(feq0+feq0)) - omegaA * (0.5*(f0-f0) - 0.5*(feq0-feq0));
            // fColl_d[ind+1*ms] = f1 - omega * (0.5*(f1+f3) - 0.5*(feq1+feq3)) - omegaA * (0.5*(f1-f3) - 0.5*(feq1-feq3));
            // fColl_d[ind+2*ms] = f2 - omega * (0.5*(f2+f4) - 0.5*(feq2+feq4)) - omegaA * (0.5*(f2-f4) - 0.5*(feq2-feq4));
            // fColl_d[ind+3*ms] = f3 - omega * (0.5*(f3+f1) - 0.5*(feq3+feq1)) - omegaA * (0.5*(f3-f1) - 0.5*(feq3-feq1));
            // fColl_d[ind+4*ms] = f4 - omega * (0.5*(f4+f2) - 0.5*(feq4+feq2)) - omegaA * (0.5*(f4-f2) - 0.5*(feq4-feq2));
            // fColl_d[ind+5*ms] = f5 - omega * (0.5*(f5+f7) - 0.5*(feq5+feq7)) - omegaA * (0.5*(f5-f7) - 0.5*(feq5-feq7));
            // fColl_d[ind+6*ms] = f6 - omega * (0.5*(f6+f8) - 0.5*(feq6+feq8)) - omegaA * (0.5*(f6-f8) - 0.5*(feq6-feq8));
            // fColl_d[ind+7*ms] = f7 - omega * (0.5*(f7+f5) - 0.5*(feq7+feq5)) - omegaA * (0.5*(f7-f5) - 0.5*(feq7-feq5));
            // fColl_d[ind+8*ms] = f8 - omega * (0.5*(f8+f6) - 0.5*(feq8+feq6)) - omegaA * (0.5*(f8-f6) - 0.5*(feq8-feq6));
        }
    }
}

void cpuCollMrt2D(int* fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *u_d, FLOAT_TYPE *v_d, FLOAT_TYPE *f_d, FLOAT_TYPE *fColl_d)
{
    int ms = depth*length;
    FLOAT_TYPE mEq[9], m[9], collision[9], f[9];

    FLOAT_TYPE r,u,v;

    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            r = rho_d[ind];
            u = u_d[ind];
            v = v_d[ind];

            // m^eq = (rho, e, epsilon, jx, qx, jy, qy, pxx, pxy)^T
            mEq[0] = r;
            mEq[1] = r * (-2. + 3. * r * (u * u + v * v));
            mEq[2] = r * (1. - 3. * r * (u * u + v * v));
            mEq[3] = r * u;
            mEq[4] = -r * u;
            mEq[5] = r * v;
            mEq[6] = -r * v;
            mEq[7] = r * (u * u - v * v);
            mEq[8] = r * u * v;

            f[0] = f_d[ind];
            f[1] = f_d[ind + ms];
            f[2] = f_d[ind + 2 * ms];
            f[3] = f_d[ind + 3 * ms];
            f[4] = f_d[ind + 4 * ms];
            f[5] = f_d[ind + 5 * ms];
            f[6] = f_d[ind + 6 * ms];
            f[7] = f_d[ind + 7 * ms];
            f[8] = f_d[ind + 8 * ms];

            // m = Mf
            for (int i = 0; i < 9; ++i) {
                m[i] = 0;
                for (int j = 0; j < 9; ++j) {
                    m[i] += velMomMap2D[i * 9 + j] * f[j];
                }
            }

            // Diff = M^-1 * S * (m - m^eq)
            for (int i = 0; i < 9; ++i) {
                collision[i] = 0;
                for (int j = 0; j < 9; ++j) {
                    collision[i] += momCollMtx2D[i * 9 + j] * (m[j] - mEq[j]);
                }
            }

            // fColl = f - M^-1 * S * (m - m^eq) >>> MRT equation
            fColl_d[ind] = f[0] - collision[0];
            fColl_d[ind + ms] = f[1] - collision[1];
            fColl_d[ind + 2 * ms] = f[2] - collision[2];
            fColl_d[ind + 3 * ms] = f[3] - collision[3];
            fColl_d[ind + 4 * ms] = f[4] - collision[4];
            fColl_d[ind + 5 * ms] = f[5] - collision[5];
            fColl_d[ind + 6 * ms] = f[6] - collision[6];
            fColl_d[ind + 7 * ms] = f[7] - collision[7];
            fColl_d[ind + 8 * ms] = f[8] - collision[8];
        }
    }
}

void cpuCollMrt3D(int* fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *u_d,
                             FLOAT_TYPE *v_d, FLOAT_TYPE *w_d, FLOAT_TYPE *f_d, FLOAT_TYPE *fColl_d)
{

    int ms = depth*length*height;
    FLOAT_TYPE mEq[19], mEq2[19], m[19], collision[19], f[19];

    FLOAT_TYPE r,u,v,w;
    FLOAT_TYPE jx, jy, jz;
    FLOAT_TYPE wej; // wxx, we

    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            r = rho_d[ind];
            u = u_d[ind];
            v = v_d[ind];
            w = w_d[ind];

            jx = r * u;
            jy = r * v;
            jz = r * w;
            wej = -475. / 63.; //wxx = 0.0;we = 0.0;

            mEq[0] = r;
            mEq[1] = -11. * r + 19. * (jx * jx + jy * jy + jz * jz);
            mEq[2] = wej * (jx * jx + jy * jy + jz * jz);
            mEq[3] = jx;
            mEq[4] = -(2. / 3.) * jx;
            mEq[5] = jy;
            mEq[6] = -(2. / 3.) * jy;
            mEq[7] = jz;
            mEq[8] = -(2. / 3.) * jz;
            mEq[9] = (2 * jx * jx - (jy * jy + jz * jz));
            mEq[10] = 0.0;
            mEq[11] = jy * jy - jz * jz;
            mEq[12] = 0.0;
            mEq[13] = jx * jy;
            mEq[14] = jy * jz;
            mEq[15] = jx * jz;
            mEq[16] = 0.0;
            mEq[17] = 0.0;
            mEq[18] = 0.0;

            //if(ind==1002)printf("i am MRT\n");

            f[0] = f_d[ind + 0 * ms];
            f[1] = f_d[ind + 1 * ms];
            f[2] = f_d[ind + 2 * ms];
            f[3] = f_d[ind + 3 * ms];
            f[4] = f_d[ind + 4 * ms];
            f[5] = f_d[ind + 5 * ms];
            f[6] = f_d[ind + 6 * ms];
            f[7] = f_d[ind + 7 * ms];
            f[8] = f_d[ind + 8 * ms];
            f[9] = f_d[ind + 9 * ms];
            f[10] = f_d[ind + 10 * ms];
            f[11] = f_d[ind + 11 * ms];
            f[12] = f_d[ind + 12 * ms];
            f[13] = f_d[ind + 13 * ms];
            f[14] = f_d[ind + 14 * ms];
            f[15] = f_d[ind + 15 * ms];
            f[16] = f_d[ind + 16 * ms];
            f[17] = f_d[ind + 17 * ms];
            f[18] = f_d[ind + 18 * ms];

            // m = Mf
            for (int i = 0; i < 19; ++i) {
                m[i] = 0.;
                for (int j = 0; j < 19; ++j) {
                    m[i] += velMomMap3D[i * 19 + j] * f[j];
                }
            }

            // m = Mf


            for (int i = 0; i < 19; ++i) {
                mEq2[i] = 0.;
                for (int j = 0; j < 19; ++j) {
                    mEq2[i] += velMomMap3D[i * 19 + j] * feqc3D(u, cx3D[j],
                                                                  v, cy3D[j], w, cz3D[j], r, w3D[j]);
                }
            }
            //if(ind==0)for (int i=0; i<19; ++i)printf("mEq0(%d)= %.14f\n",i,mEq2[i]-mEq[i]);


            // Diff = M^-1 * S * (m - m^eq)
            for (int i = 0; i < 19; ++i) {
                collision[i] = 0.;
                for (int j = 0; j < 19; ++j) {
                    collision[i] += momCollMtx3D[i * 19 + j] * (m[j] - mEq[j]);
                }
            }

            // fColl = f - M^-1 * S * (m - m^eq) >>> MRT equation
            fColl_d[ind + 0 * ms] = f[0] - collision[0];
            fColl_d[ind + 1 * ms] = f[1] - collision[1];
            fColl_d[ind + 2 * ms] = f[2] - collision[2];
            fColl_d[ind + 3 * ms] = f[3] - collision[3];
            fColl_d[ind + 4 * ms] = f[4] - collision[4];
            fColl_d[ind + 5 * ms] = f[5] - collision[5];
            fColl_d[ind + 6 * ms] = f[6] - collision[6];
            fColl_d[ind + 7 * ms] = f[7] - collision[7];
            fColl_d[ind + 8 * ms] = f[8] - collision[8];
            fColl_d[ind + 9 * ms] = f[9] - collision[9];
            fColl_d[ind + 10 * ms] = f[10] - collision[10];
            fColl_d[ind + 11 * ms] = f[11] - collision[11];
            fColl_d[ind + 12 * ms] = f[12] - collision[12];
            fColl_d[ind + 13 * ms] = f[13] - collision[13];
            fColl_d[ind + 14 * ms] = f[14] - collision[14];
            fColl_d[ind + 15 * ms] = f[15] - collision[15];
            fColl_d[ind + 16 * ms] = f[16] - collision[16];
            fColl_d[ind + 17 * ms] = f[17] - collision[17];
            //        fColl_d[ind  +  18  *ms]  =  f[  18  ]  -  collision[  18  ];
            //        if(ind==1123){
            //        	printf("MRT f_coll f0: %.14f f1: %.14f f2: %.14f f3: %.14f f4: %.14f f5: %.14f f6: %.14f f7: %.14f f8: %.14f "
            //					"f9: %.14f f10: %.14f f11: %.14f f12: %.14f f3: %.14f f4: %.14f f15: %.14f f16: %.14f f17: %.14f f18: %.14f\n",
            //					  fColl_d[ind+ 0*ms],fColl_d[ind+ 1*ms],fColl_d[ind+ 2*ms],fColl_d[ind+ 3*ms],fColl_d[ind+ 4*ms],fColl_d[ind+ 5*ms],fColl_d[ind+ 6*ms],fColl_d[ind+ 7*ms], fColl_d[ind+ 8*ms], fColl_d[ind+ 9*ms],
            //					fColl_d[ind+10*ms],fColl_d[ind+11*ms],fColl_d[ind+12*ms],fColl_d[ind+13*ms],fColl_d[ind+14*ms],fColl_d[ind+15*ms], fColl_d[ind+16*ms], fColl_d[ind+17*ms], fColl_d[ind+18*ms]);
            //
            //        }
        }
    }
}

void cpuCollMrt3D_short(int* fluid_d, FLOAT_TYPE *rho_d, FLOAT_TYPE *u_d,
                                   FLOAT_TYPE *v_d, FLOAT_TYPE *w_d, FLOAT_TYPE *f_d, FLOAT_TYPE *fColl_d)
{

    int ms = depth*length*height;
    FLOAT_TYPE mEq[19], m[19], collision[19];

    FLOAT_TYPE r,u,v,w;

    for (int ind = 0; ind < ms; ind++) {
        if (fluid_d[ind] == 1) {
            r = rho_d[ind];
            u = u_d[ind];
            v = v_d[ind];
            w = w_d[ind];


            // m = Mf
            for (int i = 0; i < 19; ++i) {
                m[i] = 0.;
                for (int j = 0; j < 19; ++j) {
                    m[i] += velMomMap3D[i * 19 + j] * f_d[ind + j * ms];
                }
            }

            // mEq = Mfeq


            for (int i = 0; i < 19; ++i) {
                mEq[i] = 0.;
                for (int j = 0; j < 19; ++j) {
                    mEq[i] += velMomMap3D[i * 19 + j] * feqc3D(u, cx3D[j],
                                                                 v, cy3D[j], w, cz3D[j], r, w3D[j]);
                }
            }

            // Diff = M^-1 * S * (m - m^eq)
            for (int i = 0; i < 19; ++i) {
                collision[i] = 0.;
                for (int j = 0; j < 19; ++j) {
                    collision[i] += momCollMtx3D[i * 19 + j] * (m[j] - mEq[j]);
                    //				if(ind == 0)printf("%f,",momCollMtx3D[i*19+j]);
                }
                //			if(ind == 0)printf("\n");
            }

            // fColl = f - M^-1 * S * (m - m^eq) >>> MRT equation
            for (int i = 0; i < 19; ++i) {
                fColl_d[ind + i * ms] = f_d[ind + i * ms] - collision[i];
            }
            if (ind == 1123) {
                //        	printf("MRT f_coll f0: %.14f f1: %.14f f2: %.14f f3: %.14f f4: %.14f f5: %.14f f6: %.14f f7: %.14f f8: %.14f "
                //					"f9: %.14f f10: %.14f f11: %.14f f12: %.14f f3: %.14f f4: %.14f f15: %.14f f16: %.14f f17: %.14f f18: %.14f\n",
                //					  fColl_d[ind+ 0*ms],fColl_d[ind+ 1*ms],fColl_d[ind+ 2*ms],fColl_d[ind+ 3*ms],fColl_d[ind+ 4*ms],fColl_d[ind+ 5*ms],fColl_d[ind+ 6*ms],fColl_d[ind+ 7*ms], fColl_d[ind+ 8*ms], fColl_d[ind+ 9*ms],
                //					fColl_d[ind+10*ms],fColl_d[ind+11*ms],fColl_d[ind+12*ms],fColl_d[ind+13*ms],fColl_d[ind+14*ms],fColl_d[ind+15*ms], fColl_d[ind+16*ms], fColl_d[ind+17*ms], fColl_d[ind+18*ms]);

            }
        }
    }
}
